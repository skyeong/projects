...
class TF4Neuro:
    ...
    def insert_data(self,fn_csv):
        ...
        self.df = pd.read_csv(fn_csv, sep=',', header=0)
        print('shape = (%d, %d)' % self.df.shape)

    def set_train_x(self, edges):
        self.nFeatures = len(edges)
        self.edges = edges
        self.train_x = self.df[edges].as_matrix()

    def set_train_t(self,targetTag):
        self.train_t = self.df[targetTag].as_matrix().reshape([len(self.df),1])

    def set_num_nodes(self,nNodes):
        self.nNodes = nNodes

    def prepare_singlelayer_session(self,mode='sigmoid'):
        mult = self.train_x.flatten().mean()
        ...
        hidden1 = tf.nn.tanh(tf.matmul(self.x,w1) + b1*mult)  # *mult is to boost speed

        w0 = tf.Variable(tf.zeros([self.nNodes, 1]))
        b0 = tf.Variable(tf.zeros([1]))
        if mode is 'sigmoid':
            p = tf.nn.softmax(tf.matmul(hidden1,w0)+b0*mult)
        elif mode is 'tanh':
            p = tf.nn.sigmoid(tf.matmul(hidden1,w0)+b0*mult)
        elif mode is 'relu':
            p = tf.nn.relu(tf.matmul(hidden1,w0)+b0*mult)
...
if __name__=="__main__":
    ...
    nNodes = 5  # define number of nodes in hidden layer
    niter = 50000

    # Create TensorFlow Class
    neuro = TF4Neuro()
    neuro.insert_data('/Users/skyeong/data/delirium/Results/delirium_network_n116.csv')
    #neuro.set_train_x(edges)
    neuro.set_train_x(['r2_108','r4_92','r10_25','r10_26','r28_116','r32_60'])
    neuro.set_train_t('Dx')
    neuro.set_num_nodes(nNodes)
    neuro.prepare_singlelayer_session('tanh')
